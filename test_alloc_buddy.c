...
int main() {
	...
	int handle = inits (4096, 8);
	printWaste(handle);
	printSpace(handle);
	
	char* buddyPointers[5*15];
	...
	while (1) {
		...
		for (i = 0; i < 5; i ++) {
			int j;
			for (j = 0; j < 15; j++) {
				buddyPointers[(i * 15) + j] =  (char*)memalloc(handle, strlen(strings[i]) + 1);
				strcpy(buddyPointers[(i * 15) + j], strings[i]);
				printf(" >>> %s\n", buddyPointers[(i * 15) + j]);
			}
		}
		
		printf("<<< Freeing them >>>\n");
		for (i = 0; i < 5 * 15; i ++) {
			memfree(buddyPointers[i]);
		}
	}
}