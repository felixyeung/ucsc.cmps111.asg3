...
int main() {
	...
	int handle = inits (4096 * 256, 2, parm2);
	printWaste(handle);
	printSpace(handle);
	
	char* slabPointers[5*15];
	...
	while (1) {
		...
		for (i = 0; i < 5; i ++) {
			int j;
			for (j = 0; j < 15; j++) {
				slabPointers[(i * 15) + j] =  (char*)memalloc(handle, strlen(strings[i]) + 1);
				strcpy(slabPointers[(i * 15) + j], strings[i]);
				printf(" >>> %s\n", slabPointers[(i * 15) + j]);
			}
		}
